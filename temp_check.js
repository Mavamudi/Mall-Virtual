        ...
        window.toggleWalkMode = function () {
            ...
            if (isWalking) {
                ...
                camera.position.set(0, 1.7, -82); // Entrada Sur (Frente al Tótem)
                controls.target.set(0, 1.80, -78); // Mirando al Tótem (Un pelito arriba)
        ...
        function createAnchorStore(posX, posZ, width, height, name = "ANCLA", idLetter = "") {
            ...
            m(width + 2, 0.2, 30, 0, height, -10, whiteMat); // Techo sólido
        ...
        function createPlanter(x, z) {
            ...
            // Follaje Orgánico (Agrupación de esferas)
            ...
            foliageConfig.forEach((f, idx) => {
                ...
                blob.scale.set(1, 0.8, 1); // Ligeramente achatadas para look más arbóreo
                gr.add(blob);
            });
        ...
        function updateKeyboardNavigation() {
            ...
            // --- MOTOR DE ESCALERAS MECÁNICAS (V3 GEOMÉTRICA) ---
        ...
        async function getStoreData(code) {
            ...
            // 2. Fallback a generador aleatorio si no está en DB
        ...
        function openModal(data) {
            ...
            // Mostrar botón de gestión solo si el usuario logueado es el dueño
        ...
        window.tenantLogin = async function() {
            ...
            if (!supabaseClient) return alert("Error de conexión");
        ...
        window.loadAdminData = async function() {
            ...
            // 2. Cargar Estadísticas de Locales
        ...
        window.assignStorePrompt = async function(email) {
            ...
            if (!uid) return;
            const code = prompt("Ingresa el CÓDIGO del local (ej: N101):");
            if (!code) return;
            
            const { error } = await supabaseClient.from('stores').update({ owner_id: uid }).eq('id', code);
            if (error) alert("Error: " + error.message);
            else {
                alert("¡Local asignado con éxito!");
                loadAdminData();
            }
        }
        ...
        window.sendStoreMessage = async function() {
            ...
            if(error) alert("Error al enviar: " + error.message);
                ...
                alert("¡Mensaje enviado al dueño del local!");
        ...
        const SB_URL = "https://kcfuixvrwbnizspgtmtr.supabase.co";
        const SB_KEY = "sb_publishable_-H23KD1xJafE_DFpBFZlyA_CL9sNlpG";
        ...
        window.startMallExperience = async function () {
            ...
            setTimeout(() => {
                ...
                // Forzar posición de inicio al entrar
                if (!isWalking) {
            ...
            if (isMember && supabaseClient) {
                ...
                if (!email || !phone) {
                    alert("Por favor, completa tu correo y celular para ser socio, o desmarca la opción.");
        ...
        function createAvatar(nickname, styleCode = "1") {
            ...
            // Cara: Ojos (Añade expresividad humana)
            ...
            handL.position.set(0, -0.33, 0); // Posición relativa al final del antebrazo
            armL.children[0].children[1].add(handL); // Añadir a la parte inferior (antebrazo)
            group.add(armL);
            ...
            legL.children[0].children[1].add(footL); // Añadir a la parte inferior
            group.add(legL);
            ...
            // Vincular metadatos para interacción
        ...
        function updateOtherPlayers() {
            Object.values(otherPlayers).forEach(p => {
                if (p.mesh) {
                    ...
                    // Aplicar bobbing visual a las partes superiores y oscilación a extremidades
                    p.mesh.children.forEach(c => {
                        ...
                        // Oscilación de brazos y piernas con articulación
                        if (["armL", "armR", "legL", "legR"].includes(c.name)) {
                            ...
                            if (c.name.startsWith("leg")) {
                                lower.rotation.x = s < 0 ? -s * 1.1 : 0; // Doblar rodilla al ir atrás
                            } else {
